...
import com.yonyou.service.SuiteService;
import com.yonyou.iuap.ucf.oe.open.isv.constants.IsvEventType;
import com.yonyou.iuap.ucf.oe.open.isv.crypto.EncryptionHolder;
import com.yonyou.iuap.ucf.oe.open.isv.crypto.IsvEventCrypto;
import com.yonyou.iuap.ucf.oe.open.isv.model.EventContent;
...
public class PushAuthController {
    ...
    @PostMapping("/api/open/auth_push")
    public String authCallbackHandler(@RequestBody EncryptionHolder holder) throws IOException {

        // 构建解密验签处理对象
        IsvEventCrypto crypto = new IsvEventCrypto(suiteService.getSuiteConfig().getSuiteSecret(), suiteService.getSuiteConfig().getEncodingAESKey(), suiteService.getSuiteConfig().getSuiteKey());
        // 验签解密后的消息体
    ...
    private void handleTicketPush(EventContent content) {
        ...
        suite.setSuiteSecret(suiteService.getSuiteConfig().getSuiteSecret());
