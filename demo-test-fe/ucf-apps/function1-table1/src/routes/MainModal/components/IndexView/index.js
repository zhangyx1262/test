...
class AddEdittable1 extends Component {
    constructor(props) {
        super(props);
        this.state = {
            rowData: {},
    ...
    componentDidMount(){
        ...
        const { table1Obj, table1Index: currentIndex } = this.props;
        // 防止网络阻塞造成btnFlag显示不正常
        this.setState({btnFlag: btnFlag});
        let rowData = {};
        try {
            // 判断是否重后端请求数据
            if (btnFlag > 0 && checkTable === "table1") {
    ...
    onSubmitEdit = () => {
        ...
        this.props.form.validateFields(async (err, values) => {
            if (!err) {
                ...
                actions.masterDetailMany.savetable1(values); //保存主表数据
                //返回
...
,
                rules: [{
                    type:'string',required: true,pattern:/\S+/ig, message: '请输入表编号',
                }],
                onChange(value) {
if(typeof rowData !== 'undefined'){
    ...
    _this.setState({
        ...
        />
        <FormError errorMsg={getFieldError('table1_no')}/>
                                </FormItem>
                                <FormItem>
        <Label >
            属性名称
        ...
        <FormControl disabled={typeof btnFlag != 'undefined' && btnFlag == 2
            ...
            {
            ...getFieldProps('table1_name', {
                validateTrigger: 'onBlur',
                initialValue: (typeof rowData === 'undefined' || typeof rowData.table1_name === 'undefined') ? "" : rowData.table1_name
,
                rules: [{
                    type:'string',required: false,pattern:/\S+/ig, message: '请输入属性名称',
                }],
                onChange(value) {
if(typeof rowData !== 'undefined'){
    let tempRow = Object.assign({},rowData,{ table1_name: value });
    _this.setState({
        rowData:tempRow
    })
}
        ...
        />
        <FormError errorMsg={getFieldError('table1_name')}/>
                                </FormItem>
                </FormList>
            </div>
        )
...
export default FormList.createForm()(AddEdittable1);
