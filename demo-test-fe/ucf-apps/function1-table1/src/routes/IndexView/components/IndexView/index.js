...
export default class IndexView extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            checkTable: "table1", //选中的表名 用于modal 弹框标记
            flag: -1, //按钮状态
        }
    ...
    onChangeTab = (tabKey) => {
        if (tabKey !== "uploadFill") { // 判断是否文件上传
            const {table1Obj, table1Index, searchParam} = this.props;
            const {pageSize} = this.props[tabKey + "Obj"];
            const {id: search_table1_no} = table1Obj.list[table1Index] || {};
            if (search_table1_no) { //如果主表有数据，子表在获取数据
                const param = {search_table1_no, pageIndex: 0, pageSize};
            }
        }
    ...
    onPrint = () => {
        const {table1Index, table1Obj} = this.props;
        const {list} = table1Obj;
        if (list.length === 0) {
            Warning('请选择需打印的数据');
            return;
        }
        const {id} = list[table1Index] || {};
    ...
    async confirmGoBack(type) {
        ...
        if (type === 1 && list.length > 0) {
            if (checkTable === "table1") { // 主表
                const {table1Index} = this.props;
                const record = list[table1Index];
                await actions.masterDetailMany.deltable1(record);
            }
        }
    }
    /**
    ...
    render() {
        ...
        let selectRow = table1Obj['list'][table1Index] || {};
        // 主表数据为空
        const table1Forbid = table1Obj.list.length > 0 ? false : true;
        let {list} = table1Obj;
        const {id} = list[table1Index] || {};
        ...
        return (
            <div className='master-detail-many'>
                <Header title='功能1'/>
                <SearchArea table1Obj={table1Obj} onRef={this.onRef}/>
                <div className='table-header'>
                    <ButtonRoleGroup funcCode="function1">
                        <Button
                            className="ml8"
                            role="add"
                            colors="primary"
                            onClick={() => this.onShowMainModal('table1', 0)}
                        >新增</Button>
                        <Button
                            className="ml8"
                            role="update"
                            shape='border'
                            disabled={table1Forbid}
                            onClick={() => _this.onShowMainModal("table1", 1)}
                        >修改</Button>
                        <Button
                            className="ml8"
                            shape='border'
                            disabled={table1Forbid}
                            onClick={() => _this.onShowMainModal("table1", 2)}
                        >详情</Button>
                        <Button
                            className="ml8"
                            role="delete"
                            shape='border'
                            disabled={table1Forbid}
                            onClick={() => _this.onClickDel("table1")}
                        >删除</Button>
                    </ButtonRoleGroup>
                </div>
                <Grid
                    ...
                    data={table1Obj.list}
                    rowKey={(r, i) => i}
                    columns={_this.table1Column}
                    getSelectedDataFunc={this.getSelectedDataFunc}
                    showHeaderMenu={true}
                    draggable={true}
                    multiSelect={false}
                    onRowClick={(record, index) => {
                        actions.masterDetailMany.updateState({table1Index: index});
                        // 根据tab 页来获取子表数据
                        const {
                            table1Obj, 
                            tabKey, 
                        ...
                        const {list} = table1Obj;
                        const {id: search_table1_no} = list[index];
                        let param = {pageIndex: 0, search_table1_no};
                    }}
                    rowClassName={(record, index, indent) => { //判断是否选中当前行
                        return table1Index === index ? "selected" : "";
                    }}
                    paginationObj={{
                        ...this.getBasicPage(table1Obj),
                        freshData: (pageSize) => {
                            _this.freshData(pageSize, "table1Obj");
                        },
                        onDataNumSelect: (index, value) => {
                            _this.onDataNumSelect(index, value, "table1Obj");
                        },
                        dataNum: 0,
                    }}
                />
                <div className="table-space"> </div>
                <div className={table1Forbid? "tabel-header-wrap-hide":"tabel-header-wrap"} >
                    <Tabs
