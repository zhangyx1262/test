...
export default {
    ...
    effects: {
        ...
        async loadList(param, getState) {
            ...
            const {result} = processData(await api.gettable1(param));  // 调用 getList 请求数据
            const {data: restable1} = result;
            const {content = []} = restable1 || {};

            if (content.length > 0) { // 获取子表数据
                const table1Obj = structureObj(restable1, param);
                actions.masterDetailMany.updateState({table1Obj}); // 更新主表数据
            } else {
                 const {
                    table1Obj,
                } = getState().masterDetailMany;
                actions.masterDetailMany.updateState({   // 如果请求出错,数据初始化
                        table1Obj: initStateObj(table1Obj),
                        table1Row: {}
                    }
                );
            }
            actions.masterDetailMany.updateState({showLoading: false, table1Index: 0});
        },
        /**
         * getSelect：保存主表数据
         * @param {*} param
         * @param {*} getState
         */
        async savetable1(param, getState) {
            actions.masterDetailMany.updateState({showLoading: true});   // 正在加载数据，显示加载 Loading 图标
            const {btnFlag} = param;
            let status = null;
            delete param.btnFlag; //删除标识字段
            if (btnFlag === 0) { // 添加数据
                const {result} = processData(await api.savetable1(param), '添加成功');
                status = result.status;
            }
            if (btnFlag === 1) { // 修改数据
                const {result} = processData(await api.updatetable1(param), '修改成功');
                status = result.status;
            }

            if (status === 'success') { // 如果不判断是会报错，param参数有错
                const {pageSize} = getState().masterDetailMany.table1Obj;
                // 带上子表信息
         ...
         * @param {*} getState
         */
        async deltable1(param, getState) {
            const {id} = param;
            const {result}=processData(await api.deltable1([{id}]), '删除成功');
            const {status}=result;
            if(status==='success'){
                // 获取表pageSize;
                const {table1Obj} = getState().masterDetailMany;
                const {pageSize} = table1Obj;
         ...
         * @param {*} getState
         */
        async querytable1(param, getState) {
