...
export default {
    ...
    effects: {
        ...
        async loadList(param, getState) {
            ...
            const {result} = processData(await api.gettable2(param));  // 调用 getList 请求数据
            const {data: restable2} = result;
            const {content = []} = restable2 || {};

            if (content.length > 0) { // 获取子表数据
                const table2Obj = structureObj(restable2, param);
                actions.masterDetailMany.updateState({table2Obj}); // 更新主表数据
            } else {
                 const {
                    table2Obj,
                } = getState().masterDetailMany;
                actions.masterDetailMany.updateState({   // 如果请求出错,数据初始化
                        table2Obj: initStateObj(table2Obj),
                        table2Row: {}
                    }
                );
            }
            actions.masterDetailMany.updateState({showLoading: false, table2Index: 0});
        },
        /**
         * getSelect：保存主表数据
         * @param {*} param
         * @param {*} getState
         */
        async savetable2(param, getState) {
            actions.masterDetailMany.updateState({showLoading: true});   // 正在加载数据，显示加载 Loading 图标
            const {btnFlag} = param;
            let status = null;
            delete param.btnFlag; //删除标识字段
            if (btnFlag === 0) { // 添加数据
                const {result} = processData(await api.savetable2(param), '添加成功');
                status = result.status;
            }
            if (btnFlag === 1) { // 修改数据
                const {result} = processData(await api.updatetable2(param), '修改成功');
                status = result.status;
            }

            if (status === 'success') { // 如果不判断是会报错，param参数有错
                const {pageSize} = getState().masterDetailMany.table2Obj;
                // 带上子表信息
         ...
         * @param {*} getState
         */
        async deltable2(param, getState) {
            const {id} = param;
            const {result}=processData(await api.deltable2([{id}]), '删除成功');
            const {status}=result;
            if(status==='success'){
                // 获取表pageSize;
                const {table2Obj} = getState().masterDetailMany;
         ...
         * @param {*} getState
         */
        async querytable2(param, getState) {
